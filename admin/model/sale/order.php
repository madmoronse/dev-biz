...
class ModelSaleOrder extends Model {
	public function addOrder($data) {
		...
		if ($country_info) {
			...
			$shipping_country = '';	
			$shipping_address_format = '{lastname} {firstname} {middlename}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{address_3}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
		}	
		...
		if ($country_info) {
			...
			$payment_country = '';	
			$payment_address_format = '{lastname} {firstname} {middlename}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{address_3}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';					
		}
	...
	public function editOrder($order_id, $data) {
		...
		if ($order_query->num_rows) {
      	...
      	$this->db->query("UPDATE `" . DB_PREFIX . "order` SET lastname = '" . $this->db->escape($data['lastname']) . "', firstname = '" . $this->db->escape($data['firstname']) . "', middlename = '" . $this->db->escape($data['middlename']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_middlename = '" . $this->db->escape($data['payment_middlename']) . "', payment_company = '" . $this->db->escape($data['payment_company']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']). "', payment_address_3 = '" . $this->db->escape($data['payment_address_3']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_postcode = '" . $this->db->escape($data['payment_postcode']) . "', payment_country = '" . $this->db->escape($payment_country) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', payment_zone = '" . $this->db->escape($payment_zone) . "', payment_zone_id = '" . (int)$data['payment_zone_id'] . "', payment_address_format = '" . $this->db->escape($payment_address_format) . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_middlename = '" . $this->db->escape($data['shipping_middlename']) . "',  shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_address_3 = '" . $this->db->escape($data['shipping_address_3']) . "', shipping_city = '" . $this->db->escape($data['shipping_city']) . "', shipping_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', shipping_country = '" . $this->db->escape($shipping_country) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', shipping_zone = '" . $this->db->escape($shipping_zone) . "', shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', shipping_address_format = '" . $this->db->escape($shipping_address_format) . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', shipping_code = '" . $this->db->escape($data['shipping_code']) . "', comment = '" . $this->db->escape($data['comment']) . "', order_status_id = '" . (int)$data['order_status_id'] . "', affiliate_id  = '" . (int)$data['affiliate_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	...
	public function deleteOrder($order_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_comments WHERE order_id = '" . (int)$order_id . "'");
	...
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT *, (SELECT CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = o.customer_id) AS customer FROM `" . DB_PREFIX . "order` o WHERE o.order_id = '" . (int)$order_id . "'");
		...
		if ($order_query->num_rows) {
			...
			if ($affiliate_info) {
				...
				$affiliate_lastname = $affiliate_info['middlename'];
			} else {
				$affiliate_lastname = '';
				$affiliate_firstname = '';
				$affiliate_middlename = '';				
			}
			...
			return array(
				...
				'affiliate_middlename'    => $affiliate_middlename,
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, o.shipping_method, o.tracking_number, (SELECT text FROM " . DB_PREFIX . "order_total WHERE order_id = o.order_id AND code='shipping' LIMIT 1) as shipping_costs, CONCAT(o.lastname, ' ', o.firstname, ' ', o.middlename) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1) AS status, (SELECT cgd.name FROM " . DB_PREFIX . "customer_group_description cgd WHERE cgd.customer_group_id = o.customer_group_id AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "' LIMIT 1) AS customer_group, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
		...
		if (!empty($data['filter_customer_group_id']) && !is_null((int)$data['filter_customer_group_id'])) {
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND CONCAT(o.lastname, ' ', o.firstname, ' ', o.middlename) LIKE '%" . $this->db->escape($data['filter_customer']) . "%'";
		}
		...
		if (!empty($data['filter_tracking_number'])) {
			$sql .= " AND o.tracking_number = '" . $this->db->escape($data['filter_tracking_number']) . "'";
		}
	...
	public function getOrderProducts($order_id) {
		...
		$query = $this->db->query("SELECT op.*,p.image, p.product_id FROM " . DB_PREFIX . "order_product op LEFT JOIN " . DB_PREFIX . "product p ON op.product_id = p.product_id WHERE order_id = '" . (int)$order_id . "'");
		return $query->rows;
	}
	...
	public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND CONCAT(lastname, ' ', firstname, ' ', middlename) LIKE '%" . $this->db->escape($data['filter_customer']) . "%'";
		}
		...
		if (!empty($data['tracking_number'])) {
			$sql .= " AND o.tracking_number = '" . (float)$data['tracking_number'] . "'";
		}
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			if ($order_status_query->num_rows) {
				...
				if ($order_status_query->row['name'] == 'Сделка завершена') {
					$message .= 'Свой отзыв о работе нашего интернет-магазина вы можете оставить здесь: http://bizoutmax.ru/index.php?route=product/isitestimonial ' . "\n\n";
				}
				
				if ($order_status_query->row['name'] == 'Товар отправлен') {
					$message .= 'Трек-номер для отслеживания Вашей посылки поступит в течение четырех дней ' . "\n\n";
				}
			}
			...
			// а как производится отправка из 1С? Через этот скрипт или напрямую?

			if ($order_info['email']=='unclefedor@mail.ru') $order_info['email']='unclefedor@mail.ru,id332979458-504880800@vkmessenger.com';
	...
	public function addOrderComments($order_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "order` SET date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_comments SET order_id = '" . (int)$order_id . "', comment = '" . $this->db->escape(strip_tags($data['comment'])) . "', date_added = NOW()");
		$order_info = $this->getOrder($order_id);
	...
	public function getOrderComments($order_id, $start = 0, $limit = 10) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$query = $this->db->query("SELECT oc.date_added, oc.comment FROM " . DB_PREFIX . "order_comments oc WHERE oc.order_id = '" . (int)$order_id . "' ORDER BY oc.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}
	
	public function getTotalOrderComments($order_id) {
	  	$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "order_comments WHERE order_id = '" . (int)$order_id . "'");
