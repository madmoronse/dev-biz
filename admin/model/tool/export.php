...
class ModelToolExport extends Model {
	...
	function storeProductsIntoDatabase( &$database, &$products ) 
	{
		...
		foreach ($products as $product) {
			...
			$sql2 = "INSERT INTO `".DB_PREFIX."product_description` (`product_id`,`language_id`,`name`,`description`,`meta_description`,`meta_keyword`,`seo_title`,`seo_h1`,`tag`) VALUES ";
			$sql2 .= "($productId,$languageId,'$productName','$productDescription','$meta_description','$meta_keywords','$seo_title','$seo_h1','$tags');";
			$database->query($sql);
			$database->query($sql2);
	...
	protected function expectedCategoriesHeading() {
		return array( "category_id", "parent_id", "name", "top", "columns", "sort_order", "image_name", "date_added", "date_modified", "language_id", "seo_keyword", "description", "meta_description", "meta_keywords", "seo_title", "seo_h1", "store_ids", "layout", "status\nenabled" );
	}
	...
	protected function populateCategoriesWorksheet( &$worksheet, &$database, $languageId, &$boxFormat, &$textFormat ) {
		...
		// ..::[Title H1]::..
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('seo_title'),32)+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('seo_h1'),32)+1);
		// ..::[#Title H1]::..
		...
		// ..::[Title H1]::..
		$this->setCell( $worksheet, $i, $j++, 'seo_title', $boxFormat );
		$this->setCell( $worksheet, $i, $j++, 'seo_h1', $boxFormat );
		// ..::[#Title H1]::..
		...
		foreach ($categories as $row) {
			...
			$this->setCell( $worksheet, $i, $j++, html_entity_decode($row['seo_title'],ENT_QUOTES,'UTF-8') );
			$this->setCell( $worksheet, $i, $j++, html_entity_decode($row['seo_h1'],ENT_QUOTES,'UTF-8') );
			$storeIdList = '';
			$categoryId = $row['category_id'];
			if (isset($storeIds[$categoryId])) {
	...
	function populateProductsWorksheet( &$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, &$textFormat )
	{
		...
		// ..::[Title H1]::..
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('seo_title'),32)+1,$textFormat);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('seo_h1'),32)+1,$textFormat);
		// ..::[#Title H1]::..
		...
		foreach ($products as $row) {
			...
			$this->setCell( $worksheet, $i, $j++, html_entity_decode($row['seo_title'],ENT_QUOTES,'UTF-8'), $textFormat );
			$this->setCell( $worksheet, $i, $j++, html_entity_decode($row['seo_h1'],ENT_QUOTES,'UTF-8'), $textFormat );
