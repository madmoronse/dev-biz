...
class ModelCatalogTestimonial extends Model {
	...
	public function addTestimonial($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial SET name='".$this->db->escape($data['name'])."', image1='".$this->db->escape($data['image1'])."', image2='".$this->db->escape($data['image2'])."', image3='".$this->db->escape($data['image3'])."', city = '".$this->db->escape($data['city'])."', status = '" . (int)$data['status'] . "',rating = '".(int)$data['rating'] . "',date_added = '" . $this->db->escape($data['date_added']) . "',email='" . $this->db->escape($data['email']) . "'");
	...
	public function editTestimonial($testimonial_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "testimonial SET name='".$this->db->escape($data['name'])."', image1='".$this->db->escape($data['image1'])."', image2='".$this->db->escape($data['image2'])."', image3='".$this->db->escape($data['image3'])."', city = '".$this->db->escape($data['city'])."', status = '" . (int)$data['status'] . "',date_added = '".$this->db->escape($data['date_added']). "',rating = '".(int)$data['rating']."',email='". $this->db->escape($data['email']) ."' WHERE testimonial_id = '" . (int)$testimonial_id . "'");
	...
	public function createDatabaseTables() {
		...
		$sql .= "`image1` text COLLATE utf8_unicode_ci NOT NULL, ";
		$sql .= "`image2` text COLLATE utf8_unicode_ci NOT NULL, ";
		$sql .= "`image3` text COLLATE utf8_unicode_ci NOT NULL, ";
