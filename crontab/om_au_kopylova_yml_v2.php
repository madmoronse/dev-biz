<?php
	...
	// http://new.outmaxshop.ru/crontab/om_au_kopylova_yml_v2.php
	...
	foreach($SKUS as $rowSKU=>$SKU) {
		...
		if(@$SKU_DATAS['quantity']>0) {
			...
			$CAT_PATH=$DB->selectRow('SELECT * FROM `oc_category_path` WHERE `category_id`=? AND `level`=?',$CAT_ID,1);
			if(!$CAT_PATH) {

				$CAT_PATH=$DB->selectRow('SELECT * FROM `oc_category_path` WHERE `category_id`=? AND `level`=?',$CAT_ID,0);
			...
			$OFFER.='    <categoryId>'.$CAT_ID.'</categoryId>'."\r\n";
