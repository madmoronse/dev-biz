...
class ModelCatalogTestimonial extends Model {
	...
	public function addTestimonial($data, $status) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial SET status = '".$status."', rating = '".$this->db->escape($data['rating'])."', name='".$this->db->escape($data['name'])."', image1='".$this->db->escape($data['image1'])."', image2='".$this->db->escape($data['image2'])."', image3='".$this->db->escape($data['image3'])."', city = '".$this->db->escape($data['city'])."', email='".$this->db->escape($data['email'])."', date_added = NOW()");
