...
class ModelModuleFilterPro extends Model {
	...
	public function getSubCategories($data) {
		$sql = "SELECT DISTINCT cd.category_id, CONCAT(cd.name, ' (', COUNT(*), ')' ) AS 'name' FROM `" . DB_PREFIX . "category` c" .
			   ...
			   " WHERE c.status=1 AND c.parent_id = '" . (int)$data['category_id'] . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW() AND p2s.store_id =" . (int)$this->config->get('config_store_id');
		if($data['manufacturer_id']) {
			$sql .= " AND p.manufacturer_id = '" . (int)$data['manufacturer_id'] . "'";
		}
    ...
    LEFT JOIN " . DB_PREFIX . "product p ON(pov.product_id = p.product_id) ";
		...
		$sql .= "LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON(p.product_id=p2s.product_id)
	WHERE ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "'  AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW() AND p2s.store_id =" . (int)$this->config->get('config_store_id');
	...
	public function getPriceLimits($data) {
		...
		$query = $this->db->query($sql);
        \Neos\debug\Debug::debugInfo($sql, true);
		return $query->row;
	}
	...
	public function getTotalProducts($data) {
		...
		if($data['discount']) {
			$sql .= " AND p.discount = " . (int) $data['discount'];
		}
		...
		if ($data['f_category_id'] == 0) {
			$sql .= " AND p.status = '1' AND p.quantity > '0' AND p.date_available >='" . date("Y-m-d" ,time()-60*60*24*31) . "' AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		}else{
			$sql .= " AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW( ) AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		}
	...
	public function getTotalTags($data) {
		...
		$sql .= " AND p2t.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW( ) AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		$sql .= ") as innertable WHERE 1 ";
		if($data['min_price'] >= 0) {
			$min_price = $this->currency->convert((int)$data['min_price'], $this->currency->getCode(), $this->config->get('config_currency'));
			$sql .= " AND realprice >=" . (int)$min_price;
	...
	public function getTotalCategories($data, $category_id) {
		...
		$sql .= " AND c.status=1 AND c.parent_id = '" . $category_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW( ) AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		$sql .= ") as innertable WHERE 1 ";
		if($data['min_price'] >= 0) {
			$min_price = $this->currency->convert((int)$data['min_price'], $this->currency->getCode(), $this->config->get('config_currency'));
			$sql .= " AND realprice >=" . (int)$min_price;
	...
	public function getTotalAttributes($data) {
		...
		$sql .= " AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW( ) AND p2a.language_id='" . (int)$this->config->get('config_language_id') . "' AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		$sql .= ") as innertable WHERE 1 ";
		if($data['min_price'] >= 0) {
			$min_price = $this->currency->convert((int)$data['min_price'], $this->currency->getCode(), $this->config->get('config_currency'));
			$sql .= " AND realprice >=" . (int)$min_price;
	...
	public function getProducts($data) {
		...
		$sql .= "SELECT DISTINCT p.product_id, pd.name, p.model, p.quantity, p.price, p.sort_order, p.date_added, p.date_modified ";
		...
		$sql .= $this->customSortOrderSelect($data);
		...
		$sql .= $this->customSortOrderJoin($data);
		...
		if($option_filters) {
			if($this->filterpro_setting['option_mode'] == 'and') {
                ...
                $sql .= " AND (" . implode(" OR ", $option_filters) . ")  AND pov.quantity > 0";
		...
		if ($data['category_id'] == 0){
			$sql .= " AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.quantity > '0' AND p.date_available >=". date("Y-m-d" ,time()-60*60*24*31) ." AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		} else {
			$sql .= " AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW( ) AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		}
		}else {
			$sql .= " AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.quantity > '0' AND p.date_available <= NOW( ) AND p2s.store_id = " . (int)$this->config->get('config_store_id');
		}
		...
		if(isset($data['sort']) && array_key_exists($data['sort'], $sort_data)) {
			$data['sort'] = $this->customSortOrderName($data, $sort_data[$data['sort']]);
			$data['order'] = $this->customSortOrderDirection($data, $data['order']);
		...
		return array($product_data, $sql);
	}

	public function getAdditionalProducts($data) {
		$customer_group_id = $this->getCustomerGroup();
	...
	public function getCountAllProducts($data) {
		$customer_group_id = $this->getCustomerGroup();
	...
	public function getCountAllAdditionalProducts($data) {
		$customer_group_id = $this->getCustomerGroup();
	...
	protected function customSortOrderSelect($data)
	{
		$name = $this->customSortOrderName($data);
		switch ($name) {
			default:
				return '';
			case 'newp_sort':
				return ", newp.sort as newp_sort";
		}
	}

	protected function customSortOrderJoin($data)
	{
		$name = $this->customSortOrderName($data);
		switch ($name) {
			default:
				return '';
			case 'newp_sort':
				return " LEFT JOIN " . DB_PREFIX . "module_newproducts as newp ON (newp.product_id=p.product_id AND newp.category_id = " . (int) $data['f_category_id'] . ")";
		}
	}

	protected function customSortOrderName($data, $default = null)
	{
		if (isset($data['f_category_id'])
			// new products categories
			&& in_array($data['f_category_id'], array(10013417, 10013418, 10013419))
		) {
			return 'newp_sort';
		}
		return $default;
	...
	protected function customSortOrderDirection($data, $default = null)
	{
		$name = $this->customSortOrderName($data);
		switch ($name) {
			default:
