...
class ModelXlsPricelistHelperModels extends Model {
    ...
    public function getCategoryPath($category_id, $path=array(), $language_id) {
        $path[]=$category_id;
        $query = $this->db->query("SELECT name, parent_id FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) WHERE c.category_id = '" . (int)$category_id . "' AND cd.language_id = '" . (int)$language_id . "' ORDER BY c.sort_order, cd.name ASC");
    ...
    public function getCategoryName($category_id) {
        ...
        $query->row['name'] =  ucfirst($query->row['name']);

        if ($query->row['parent_id']) {
            return $this->getCategoryName($query->row['parent_id'], $this->config->get('config_language_id')) . $this->language->get('text_separator') . $query->row['name'];
        } else {
    ...
    public function getCategorySortName($category_id) {
        $query = $this->db->query("SELECT name, parent_id FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description cd ON (c.category_id = cd.category_id) WHERE c.category_id = '" . (int)$category_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY c.sort_order, cd.name ASC");

        $query->row['name'] =  ucfirst($query->row['name']);

        if ($query->row['parent_id']) {
            if ($query->row['name'] == '���������'){$pref = 1;}
            if ($query->row['name'] == '������'){$pref = 2;}
            if ($query->row['name'] == '������ �����'){$pref = 3;}
            if ($query->row['name'] == '����������'){$pref = 4;}
            if ($query->row['name'] == '�����-����!'){$pref = 5;}
            if (isset($pref)){$query->row['name'] = $pref . $query->row['name'];}
            return $this->getCategorySortName($query->row['parent_id'], $this->config->get('config_language_id')) . $this->language->get('text_separator') . $query->row['name'];
        } else {
            return $query->row['name'];
        }
    }
