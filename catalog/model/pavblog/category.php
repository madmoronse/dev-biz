	...
	public function genTree( $parent, $level , $class="" ){
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				$output .='<li class="img_item">';
					//$output .= '<a href="'.$this->url->link('pavblog/category',"id=".$menu['category_id']).'" title="'.$menu['title'].'">'.$menu['title']."</a>";
					$output .= '<a href="'.$this->url->link('pavblog/category',"id=".$menu['category_id']).'" title="' . $menu['title'] . '"><div class="caption"><h3>' . $menu['title'] . '</h3></div><img src="./image/'.$menu['image'].'" title="' . $menu['title'] . '" alt="' . $menu['title'] . '"></a>';
