...
class ModelShippingCdek extends Model {
	...
	function getQuote($address) {
		...
		if ($to_data && $address['city']) {
			...
			foreach ($cities as $city) {
				...
				$cdek_cities = $this->getURL('http://api.cdek.ru/city/getListByTerm/json.php?q=' . urlencode($city) . '&name_startsWith=' . urlencode($city), new parser_json());
								...
								if (!empty($results)) {
									...
									foreach ($results as $shipping_info) {
										...
										if (array_key_exists($shipping_info['tariffId'], $cdek_tariff_list)) {
											...
											$price = ceil($price / 10) * 10; // NEOS - round to 10th 
											...
											if (in_array($tariff_info['mode_id'], array(2, 4))) {
												...
												if ($this->config->get('cdek_show_pvz')) {
													...
													foreach ($pvz_list as $pvz_info) {
														$names[$pvz_info['code']] = $description . ' Пункт выдачи заказов: ' . $pvz_info['address'];
													}
		...
		if (!$quote_data && !empty($empty_info['use']) && $_SESSION['customer_id'] == 3589) {
	...
	private function getPVZList($city_id, $weight = 0) {
		...
		$pvz_list_data = $this->getURL('https://integration.cdek.ru/pvzlist/v1/xml?cityid=' . $city_id, new parser_xml());
