<?php
class ControllerCheckoutfRegister extends Controller {
  	public function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkoutf/register.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/checkoutf/register.tpl';
		} else {
			$this->template = 'default/template/checkoutf/register.tpl';
		}
	...
	public function testemail(){
		$this->load->model('account/customer');
		if ($this->model_account_customer->getTotalCustomersByEmail($_POST["newemail"])) {
			echo "<span class='error'>Такой e-mail уже зарегистрирован!</span>";
		}
	}

	public function addnewuser() {

		$this->db->query("INSERT INTO " . DB_PREFIX . "new_customers SET name = '" . $this->db->escape($_POST["newname"]) . "', email = '" . $this->db->escape($_POST["newemail"]) . "', telephone = '" . $this->db->escape($_POST["newtelephone"]) . "'");

	}

	public function remnewuser() {

		$this->db->query("DELETE FROM " . DB_PREFIX . "new_customers WHERE email = '" . $this->db->escape($_POST["newemail"]) . "'");
