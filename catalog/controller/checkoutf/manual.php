<?php
class ControllerCheckoutfManual extends Controller {
	public function index() {
		$this->language->load('checkoutf/manual');
		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($this->cart->hasShipping()) {
				...
				if (!isset($json['error']['shipping'])) {
					...
					$address_data = array(
						...
						'address_3'      => $this->request->post['shipping_address_3'],
			...
			if (!isset($json['error']['payment'])) {
				...
				$address_data = array(
					...
					'address_3'      => $this->request->post['payment_address_3'],
