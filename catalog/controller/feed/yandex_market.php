...
class ControllerFeedYandexMarket extends Controller {
	...
	public function index() {
		if ($this->config->get('yandex_market_status')) {
			...
			$allowed_categories = '';
			$base_dir = realpath(DIR_APPLICATION . '../price') . '/';
			$export_options = array();
			//$this->response->addHeader('Content-Type: application/xml');

			//$filename = DIR_DOWNLOAD . 'offers.yml';
			switch ($this->request->get['target']) {
				case 'all_pictures':
					$filename = $base_dir . 'drop_05b416151423d1ea695e17b3f9b5b17a.yml';
					$export_options['all_pictures'] = true;
					$export_options['show_description'] = true;
					$export_options['show_url'] = true;
					break;
				default:
					$filename = $base_dir . 'drop_hhufhfw9f874h2f427894fh7.yml';
					break;
			}
			...
			if (!is_file($filename) || time() - @filemtime($filename) > 60*30) { // one time per half of hour
				...
				foreach ($products as $product) {
					...
					if (isset($export_options['show_url'])) {
					...
					if (isset($export_options['show_description'])) {
					...
					if ($product['image']) {
						...
						$filetype =  end(explode(".", $image_path));

						$new_image_path = "image/data/products-drop/" . $data['id'] . "/1." . $filetype;
						...
						//$data['picture'] = "./image/data/products/" .$product['product_id']."/1.jpg";
					}
					if (isset($export_options['all_pictures'])) {
						// Get Images
					...
					if ($options){
						foreach ($options[0]['option_value'] as $cur_option) {
							if ($cur_option['quantity'] > 5) {
								$cur_option['quantity'] = 5;
							}
							$offer['options']['options'] .= $cur_option['name'] . "-" . $cur_option['quantity'] . ";";
						}
						$tags .= $this->array2Tag($offer['options']);
						unset($offer['options']);
					}
...
//		}
		...
		foreach ($allowed_tags as $key => $value) {
			// Neos - Handle arrays
			if (is_array($data[$key]) && in_array($key, array('picture'))) {
				foreach ($data[$key] as $data_key => $data_value) {
					$offer['data'][$key][$data_key] = $this->prepareField($data_value);
				}
			} else {
	 ...
	 * @return string
	 */
	private function array2Tag($tags, $tag_name = null) {
		$retval = '';
		foreach ($tags as $key => $value) {
			// Neos - Handle arrays
			if (is_array($value) && in_array($key, array('picture'))) {
				$retval .= $this->array2Tag($value, $key);
			} else {
				$retval .= '<' . ($tag_name ? $tag_name : $key) . '>' . $value . '</' . ($tag_name ? $tag_name : $key) . '>' . $this->eol;
			}
		}
