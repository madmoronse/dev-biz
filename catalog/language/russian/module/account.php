<?php
// Heading
$_['heading_title']    = 'Личный Кабинет';

// Text
$_['text_register']    = 'Регистрация';
$_['text_login']       = 'Вход';
$_['text_logout']      = 'Выход';
$_['text_forgotten']   = 'Забыли пароль?';
$_['text_account']     = 'Моя информация';
$_['text_edit']        = 'Изменить контактную информацию';
$_['text_password']    = 'Пароль';
$_['text_address']     = 'Мои адреса';
$_['text_comment']     = 'Шаблоны комментариев';
$_['text_wishlist']    = 'Закладки';
$_['text_order']       = 'История заказов';
$_['text_download']    = 'Файлы для скачивания';
$_['text_return']      = 'Возвраты';
$_['text_transaction'] = 'История финансовых операций';
$_['text_teacher']      = 'Мои ученики';
$_['text_newsletter']  = 'E-Mail рассылка';
$_['text_tracking_number']    = 'Почтовые треки';
$_['text_invoice']    = 'Отчеты по продажам';
?>
