  ...
  <script type="text/javascript">
    ...
    var drawResult = function() {
	  ...
	  $.ajax({
		...
		"type":"POST",
		"data": {"query": setGeoData(), "context":"checkoutf"}
	  });
	  // Neos DEBUG -- END
  ...
  <? /*  <tr>
    ...
    */?>
      <div class="payment-form">
            ...
            $(document).ready(function() {
              $( "#np" ).autocomplete({
                source: function( request, response ) {
                  $.ajax( {
                    url: "index.php?route=checkoutf/payment_address/address",
                    dataType: "json",
                    data: {
                      q: request.term,
                      thing: 'np'
        ...
        if($this->session->data['shipping_address_is_user']=='existing') {
          $address_1 = $this->session->data['address_change']['address_1'];
          $address_2 = $this->session->data['address_change']['address_2'];
          $address_3 = $this->session->data['address_change']['address_3'];
          $address_4 = $this->session->data['address_change']['address_4'];
          $postcode = $this->session->data['address_change']['postcode'];
        }
        else {
          $address_1 = $addresses ? $addr['address_1'] : "";
          $address_2 = $addresses ? $addr['address_2'] : "";
          $address_3 = $addresses ? $addr['address_3'] : "";
          $address_4 = $addresses ? $addr['address_4'] : "";
          $postcode = $addresses ? $addr['postcode'] : "";
        }
        ?>
        <div class="payment-form__item">
          <span class="required">*</span><label for="address_1"><?php echo $entry_address_1; ?></label>
          <input type="text" id="address_1" name="address_1" autocomplete="off" value="<?=$address_1?>" class="large-field" />
        </div>
        <div class="payment-form__group">
          <div class="payment-form__item inline-item">
            ...
            <input type="text" id="address_2" name="address_2" autocomplete="off" value="<?=$address_2?>" class="large-field" />
          </div>
          <div class="payment-form__item inline-item">
            <label for="address_3"><?php echo $entry_address_3; ?></label>
            <input type="text" id="address_3" name="address_3" autocomplete="off" value="<?=$address_3?>" class="large-field" />
          </div>
          <div class="payment-form__item inline-item">
            <label for="address_4"><?php echo $entry_address_4; ?></label>
            <input type="text" id="address_4" name="address_4" autocomplete="off" value="<?=$address_4?>" class="large-field" />
          </div>
        </div>
        <div class="payment-form__item">
          <label for="postcode"><?php echo $entry_postcode; ?></label>
          <input type="text" id="postcode" name="postcode" autocomplete="off" value="<?=$postcode?>" class="large-field" />
...
$('.checkout-content select[name=\'zone_id\']').bind('change', function() {
  return;
	...
	$.ajax({
		url: 'index.php?route=checkoutf/checkout/zone&zone_id=' + this.value,
		dataType: 'json',
		beforeSend: function() {
