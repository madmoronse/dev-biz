...
$('#next-step').on('click',function(){
  var isProduction = window.location.host.match(/outmaxshop.ru/);
  ...
  switch (action) {
        ...
        success: function(json) {
          ...
          if (json['redirect']) {			
            ...
            $.ajax({
              ...
              error: function(xhr, ajaxOptions, thrownError) {
                Rollbar.error("Checkout - Couldn't load step2: " + xhr.statusText + " " + xhr.responseText);
                // alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
              }
            });
          }
          ...
          if(json['error']){
            ...
            if (json['error']['city_not_found']) {
              ...
              $.ajax({
                ...
                dataType: "json",
                data: {
                  q: [npval, $('#address_1').val(), $('#address_2').val(), $('#address_4').val()].join(', '),
                  thing: 'np1'
                },
                success: function( data ) {
                    $('.warning, .error').remove();
                    if (data.error && data.error.city_not_found) {
                        $("input[name='np']").after($('<p></p>').addClass('error').text(data.error.city_not_found));
                        return false;
                    ...
                    dataitems.forEach(function(item, i, dataitems) {
                      htmlData += '<li data-cachehash="' + data.cachehash + '">'+ item +'</li>';
                    });
                    htmlData += '</ul>';

                    $('#overlayData').html(htmlData);
                    $('.overlay-data').show();

                    $('#overlayData ul li').on('click', function(){
                      $('.overlay-data').hide();
                      var np_text = $(this).text();
                      var hash = $(this).data('cachehash');
                      //Add to DB
                      $.ajax({
                        url: "index.php?route=checkout/payment_address/address",
                        dataType: "json",
            ...
            $.ajax({
              ...
              success: function(html) {
                ...
                $("span.info").text('Нажимая кнопку «Оформить», Вы даёте согласие на хранение и обработку Ваших персональных данных');
                $('#prev-step').attr('act','step-2');
                $('.checkout-title').text('Оформление заказа: Шаг 3 из 3 - Выберите способ оплаты и доставки');
                $('.overlay').hide();
    ...
    break;
    case "confirm":    	
      var inputway = $("input:radio[name='shipping_method']:checked").val();
