...
class ModelPavblogblog extends Model { 
	...
	public function getBlog( $blogId ){
		...
		# BEGIN JNX
		$sql_images = "SELECT * FROM ".DB_PREFIX."pavblog_images WHERE news_id=".$blogId . " ORDER BY `original_name`, `id` DESC";
		$query_images = $this->db->query( $sql_images );
	    $data['blog']['images'] = $query_images->rows;
	    # END JNX
	...
	public function saveData( $data ){
		...
		if( isset($data["pavblog_blog_description"]) ){ 
			...
			foreach( $languages as $language ){
				$sql = "INSERT INTO ".DB_PREFIX ."pavblog_blog_description(`language_id`, `blog_id`,`title`,`description`,`content`,`item_link`,`item_button_text`,`blog_products`)
							...
							.$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['description'])."','".$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['content'])."','".$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['item_link'])."','".$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['item_button_text'])."','".$this->db->escape($data["pavblog_blog_description"][$language['language_id']]['blog_products'])."') ";
