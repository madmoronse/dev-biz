        ...
        <table class="list">
          <thead>
            <tr>
              ...
              <td style="width:200px">Способ оплаты</td>
            ...
            <tr class="filter">
              ...
              <td><select name="filter_order_status_id">
				...
				<td><select name="filter_customer_group_id">
                  ...
                  <option value="<?php echo $customer_group['customer_group_id']; ?>" selected="selected"><?php echo $customer_group['description']; ?></option>
                  <?php } else { ?>
                  <option value="<?php echo $customer_group['customer_group_id']; ?>"><?php echo $customer_group['description']; ?></option>
                  <?php } ?>
                  <?php } ?>
                </select></td>
			  ...
			  <td><input type="text" name="filter_tracking_number" value="<?php echo $filter_tracking_number; ?>" /></td>
              <td align="right"><a onclick="filter();" class="button"><?php echo $button_filter; ?></a></td>
            </tr>
            <?php if ($orders) { ?>
            <?php foreach ($orders as $order) { ?>
            <tr id="black_border" class="white<?php if ($order['status'] == 'Товар отправлен' or $order['status'] == 'Товар отправлен, трек-код выдан') { echo ' green"';} else
				if ($order['status'] == 'Ожидается оплата' or $order['status'] == 'Ожидание') { echo ' yellow"';} else
				if ($order['status'] == 'Заказ принят') { echo ' pink"';} else
				if ($order['status'] == 'Доставлено') { echo ' blue"';} else
				if ($order['status'] == 'В обработке') { echo ' orange"';} else	echo '"';		
			?>>
			  ...
			  <td class="left"><?php echo $order['shipping_cost']; ?></td>
			  <td class="left"><?php echo $order['shipping_method']; ?></td>
              <td class="left"><?php echo $order['date_added']; ?></td>
              <td class="left"><?php echo $order['date_modified']; ?></td>
			   <td <?php if (strlen ($order['tracking_number']) != 14) {echo 'class="editable"';} ?> id="<?php echo $order['order_id']; ?>"><?php echo $order['tracking_number']; ?></td>
...
function filter() {
	...
	var filter_tracking_number = $('input[name=\'filter_tracking_number\']').attr('value');
...
.click(function(event) {
  ...
  .blur(function(event) {// end the inline editing
		...
		$.ajax({
		url: 'index.php?route=sale/order/trackingnumber&token=<?php echo $token; ?>&order_id='+$editable.attr('id'),
		type: 'post',
		dataType: 'json',
		data: 'tracking_number=' + encodeURIComponent($editable.find(':first-child').val()),
		beforeSend: function() {
		},
		complete: function() {			
		},
		success: function(json) {
			...
			if (edited.length == 14) {
	$.ajax({
		url: 'index.php?route=sale/order/history&token=<?php echo $token; ?>&order_id='+$editable.attr('id'),
		type: 'post',
		dataType: 'html',
		data: 'order_status_id=16'+ '&notify=1' + '&comment=' + encodeURIComponent('Ваш трек-код для отслеживания местонахождения почтовой посылки: ') + encodeURIComponent($editable.find(':first-child').val())+ encodeURIComponent('\n\nСайт для отслеживания: http://www.russianpost.ru/tracking20/'),
		beforeSend: function() {	
		...
		success: function(html) {
			...
			$editable.children().replaceWith('<em class="ajax">Отправляем ... </em>');
