...
class ControllerSaleCustomer extends Controller { 
	private $error = array();
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if ($this->request->post['manager'] == '') {
                $this->load->model('user/user');
                $suid = $this->session->data['user_id'];
				$user_info = $this->model_user_user->getUser($suid);
				
				if ( $user_info['user_group_id'] == 16 or $user_info['user_group_id'] == 17 ){
					$this->request->post['manager'] = $user_info['firstname'];
				}
	...
	public function approve() {
  	...
  	protected function getForm() {
		...
		if (isset($this->error['address_middlename'])) {
		$this->data['error_address_middlename'] = $this->error['address_middlename'];
		} else {
		$this->data['error_address_middlename'] = '';
		}
		...
		if (isset($this->error['address_address_3'])) {
			$this->data['error_address_address_3'] = $this->error['address_address_3'];
		} else {
			$this->data['error_address_address_3'] = '';
		}
        ...
        if (isset($this->request->post['teacher'])) {
            $this->data['teacher'] = $this->request->post['teacher'];
        } elseif (!empty($customer_info)) {
            $this->data['teacher'] = $customer_info['teacher_id'];
        } else {
            $this->data['teacher'] = '';
        }
    	...
    	if (isset($this->request->post['qiwi'])) {
			...
			$this->data['qiwi'] = $customer_info['qiwi'];
		} else {
      		$this->data['qiwi'] = '';
    	}

    	if (empty($this->data['qiwi'])) {
    		$this->data['non_qiwi'] = 'checked';
    	} else {
    		$this->data['non_qiwi'] = '';
    	}

		if ($customer_info['manager'] != '') {
		...
		if ( $user_info['user_group_id'] == 16 ){
			$allUsers = $this->model_user_user->getUsers();
			foreach ($allUsers as $User){
				if ( $User['user_group_id'] == 16 or $User['user_group_id'] == 17 ) {
					$this->data['managers'][] = $User['firstname'];
				}
			}
		} 
  	...
  	protected function validateForm() {
		...
		$teacher = $this->model_sale_teacher->getTeacher($this->request->post['teacher']);

		if ($this->request->get['customer_id'] == $teacher['customer_id']) {
			$this->error['warning'] = $this->language->get('error_teacher');
		}
    	...
    	if (((utf8_strlen($this->request->post['qiwi']) < 10) || (utf8_strlen($this->request->post['qiwi']) > 32)) && (!isset($this->request->post['non_qiwi']))) {
		...
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {
				...
				if ((utf8_strlen($value['middlename']) < 1) || (utf8_strlen($value['middlename']) > 32)) {
					$this->error['address_middlename'][$key] = $this->language->get('error_middlename');
				}				
				...
				if ((utf8_strlen($value['address_2']) < 1) || (utf8_strlen($value['address_2']) > 128)) {
					$this->error['address_address_2'][$key] = $this->language->get('error_address_2');
				}				
				
				if ((utf8_strlen($value['address_3']) < 1) || (utf8_strlen($value['address_3']) > 128)) {
					$this->error['address_address_3'][$key] = $this->language->get('error_address_3');
				}
