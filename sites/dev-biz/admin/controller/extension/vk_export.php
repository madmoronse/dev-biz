...
class ControllerExtensionVKExport extends Controller {
    ...
    public function check_app_security() {
		...
		try {
			...
			if ($e->getCode() == 17) {
				...
				$exp = '/<span class="field_prefix"><span class="phone_number">([\d\s-+]*)<\/span>[\n\s]*';
				$exp .= '<input type="text" class="textfield field_inline" name="code" style="width:75px;" \/>[\n\s]*';
				$exp .= '<span class="field_prefix">([\s\d-]*)<\/span><\/span>/';
				if (preg_match($exp, $page, $matches) && preg_match('/action="([^"]*?)"/', $page, $url)) {
                    ...
                    $response = 'Ошибка при проверке номера телефона.';
                }
			}
			else {
				$response = $e;
    ...
    private function generatePlaceholders($product_id, $p) {
        ...
        foreach ($options as $opt) {
			...
			if ($opt_val['quantity'] > 5){
			$option_values[$opt_val['option_value_id']] = $this->model_catalog_vk_export->getProductOptionValueName($opt_val['option_value_id']) . '-' . '>5 шт';
			} else
                $option_values[$opt_val['option_value_id']] = $this->model_catalog_vk_export->getProductOptionValueName($opt_val['option_value_id']) . '-' . $opt_val['quantity'] . ' шт';
            }
            sort($option_values);
            $options_output .= implode("\n", $option_values) . PHP_EOL;
        }
    ...
    private function makeLoad($vkApi, $product_id, $category_id, $p, $mode) {
        ...
        if ($this->config->get('vk_export_mode_desc')) {
            ...
            $user_replacements_keys = explode(";", $user_replacements_keys);
            $user_replacements_values = explode(";", $this->config->get('vk_export_user_replacements_values'));
        ...
        foreach ($images_in as $img) {
			...
			//Штампуем временные файлы 
			$stamp = imagecreatefrompng('../image/outmaxwater.png');
			...
			imagecopy($im, $stamp, ($ix - $sx)/2, ($iy - $sy)/2, 0, 0, imagesx($stamp), imagesy($stamp));
			
			//Создаем временные файлы со штампами 	
			imagepng($im, $img.'.png');
        ...
        /*
		//Удаляем временные файлы со штампами 
		foreach ($images_out as $value){
