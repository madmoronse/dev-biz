<?php
class ControllerProductXlsPricelistOpt extends Controller {
	...
	$filename = str_replace('/catalog/', '', DIR_APPLICATION) . "/price/Price_Opt.xlsx";
	if (time() - @filemtime($filename) < 60*2) exit();
        ...
        foreach($stores as $store){
            ...
            foreach($cust_groups as $cust_group){
                ...
                ///////////
				$skolko_razmerov = 32;	// размерный рад (от самого маленького, до самого крупного)
				
				
                $this->delim=0;

                for($i=0; $i<$this->poles_count; $i++){
				if($this->xls_pricelist_poles[$i]['type']=='image' or $this->xls_pricelist_poles[$i]['type']=='name'){
                    ...
                    $this->width += (int)$this->xls_pricelist_poles[$i]['dlina'];
						...
						$this->sheet->getColumnDimension($this->sheet->getCellByColumnAndRow($i + $skolko_razmerov, 1)->getColumn())->setWidth((int)$this->xls_pricelist_poles[$i]['dlina']);
						$this->width += (int)$this->xls_pricelist_poles[$i]['dlina'];
					}
                }
                ...
                if($this->xls_pricelist_logo){
                    ...
                    if($logo){
                        ...
                        $columnsize = ($this->sheet->getColumnDimensionByColumn(1)->getWidth())*7+10;
                ...
                //$this->sheet->setCellValueByColumnAndRow(0+$this->delim, 3, $this->xls_pricelist_email?$this->xls_pricelist_email:'e-mail:'.$this->config->get('config_email'));

                $this->sheet->setCellValueByColumnAndRow(0+$this->delim, 3, $this->xls_pricelist_email?$this->xls_pricelist_email:'e-mail: outmaxshop.ru@gmail.com');


                $this->sheet->getStyle($this->sheet->getCellByColumnAndRow(0+$this->delim, 3)->getCoordinate())->applyFromArray($f_email);
                $this->sheet->setCellValueByColumnAndRow(0+$this->delim, 4, $this->xls_pricelist_link?$this->xls_pricelist_link:$this->config->get('config_url'));
                $this->sheet->getStyle($this->sheet->getCellByColumnAndRow(0+$this->delim, 4)->getCoordinate())->applyFromArray($f_link);
                $this->sheet->getCellByColumnAndRow(0+$this->delim, 4)->getHyperlink()->setUrl($this->config->get('config_url'));
                ...
                $this->sheet->mergeCells($this->sheet->getCellByColumnAndRow(0+$this->delim, 1)->getCoordinate().':'.$this->sheet->getCellByColumnAndRow($this->poles_count - 1+$this->delim, 1)->getCoordinate());
				...
				//вывод шапки таблицы с товаром
                for($j=0; $j<$this->poles_count; $j++){
                    if($this->xls_pricelist_poles[$j]['type']=='image'){
                        $this->load->model('tool/image');
						$this->sheet->setCellValueByColumnAndRow(0, 7, $this->xls_pricelist_poles[$j]['name'][$this->xls_pricelist_language['language_id']]);
						$this->sheet->getStyle($this->sheet->getCellByColumnAndRow(0, 7)->getCoordinate())->applyFromArray($fu);
						$this->sheet->setCellValueByColumnAndRow(0, 8, '');
						$this->sheet->getStyle($this->sheet->getCellByColumnAndRow(0, 8)->getCoordinate())->applyFromArray($fe);
                    }else{
					
						if($this->xls_pricelist_poles[$j]['type']=='name'){
							$this->sheet->setCellValueByColumnAndRow(1, 7, $this->xls_pricelist_poles[$j]['name'][$this->xls_pricelist_language['language_id']]);
							$this->sheet->getStyle($this->sheet->getCellByColumnAndRow(1, 7)->getCoordinate())->applyFromArray($fu);
							$this->sheet->setCellValueByColumnAndRow(1, 8, '');
							$this->sheet->getStyle($this->sheet->getCellByColumnAndRow(1, 8)->getCoordinate())->applyFromArray($fe);
						} else{
						
							$this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, 7, $this->xls_pricelist_poles[$j]['name'][$this->xls_pricelist_language['language_id']]);
							$this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, 7)->getCoordinate())->applyFromArray($fu);
							$this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, 8, '');
							$this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, 8)->getCoordinate())->applyFromArray($fe);
							}
						}
				}
				//вывод размеров
					$this->sheet->mergeCells($this->sheet->getCellByColumnAndRow(2, 7)->getCoordinate().':'.$this->sheet->getCellByColumnAndRow(1 + $skolko_razmerov, 7)->getCoordinate());
					$size_column_name = "Размеры в наличии";
					$this->sheet->setCellValueByColumnAndRow(2, 7, $size_column_name); 

					for($r=28; $r<=59; $r++){

							$this->sheet->getStyle($this->sheet->getCellByColumnAndRow($r-26, 7)->getCoordinate())->applyFromArray($fu);

							$this->sheet->setCellValueByColumnAndRow($r-26, 8, $r);

							$this->sheet->getStyle($this->sheet->getCellByColumnAndRow($r-26, 8)->getCoordinate())->applyFromArray($fe);	

							$this->sheet->getColumnDimension($this->sheet->getCellByColumnAndRow($r-26, 1)->getColumn())->setAutoSize(true);							
							
					}
					
					
				//Конец вывода размеров
				for ($i=1; $i<=6; $i++){
					$this->sheet->getRowDimension($i)->setOutlineLevel(1);
					$this->sheet->getRowDimension($i)->setVisible(true);
					$this->sheet->getRowDimension($i)->setCollapsed(false);
				}

				$this->sheet->freezePane('A9');	
				
				//конец вывода шапки таблицы с товаром
                ////
                ...
                $allcats=explode("_", $this->xls_pricelist_category);

                $allnum=count($allcats);
                for ($all=0; $all<$allnum; $all++){
                    $iter=0;
                    foreach($allcats as $category_id){

                        $cat = $this->model_xls_pricelist_helper_models->getCategory($category_id, $this->xls_pricelist_language['language_id']);
                        if ($all==0) {
                            if ($cat['name'] == "Кроссовки") {
                ...
                if (count($allcats)>0){
                    ...
                    foreach ($categories as $all_category) {
                        ...
                        $customer_group_id = '';
                        $data = array(
                            'filter_category_id' => $all_category['category_id'],
                            'sort' => $sort,
                            'order' => $order,
                            'start' => 0,
                            'limit' => 1000000000,
                        ...
                        $CurProds = $this->model_xls_pricelist_helper_models->getProducts($data);
                        foreach ($CurProds as $curprod) {
                            $f_prods[] = $curprod['product_id'];
                        }
                        $f_prods = array_unique($f_prods);
                        foreach ($f_prods as $curprod) {
                            $prod_cats = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$curprod . "'");
                            foreach ($prod_cats->rows as $allcat) { // � ����� ���� ������� ��������� ������
                                if ($allcat['main_category'] == 1 ) { // ���� �������
                                    $all_prods[$this->model_xls_pricelist_helper_models->getCategoryName($allcat['category_id'])][]=$curprod;
                                    break;
                                }
                            }
                        }
                    ...
                    ksort($all_prods);
                    //$all_prods = array_unique( $all_prods);
                    foreach ($all_prods as $result1) {
                        foreach ($result1 as $result){
                            $all_prod_cats = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$result . "'");


                            foreach ($all_prod_cats->rows as $allcat) { // � ����� ���� ������� ��������� ������
                                if ($allcat['main_category'] == 1) { // ���� �������
                                    $all_main_cats['category_id'][] = $allcat['category_id'];
                                    break;
                                }
                            }
                        }
                    }
                    $filtred_categories = array_unique($all_main_cats['category_id']);

                }
                //=======================================

                foreach($filtred_categories as $filt_category_id){
                    $fcat=$this->model_xls_pricelist_helper_models->getCategory($filt_category_id, $this->xls_pricelist_language['language_id']);
                    if($fcat)
                        $fcategories[] = $fcat;
                }

                foreach ($fcategories as $filtred_category) {

                    $CCategory[] = $this->model_xls_pricelist_helper_models->getCategory($filtred_category, $this->xls_pricelist_language['language_id']);

                    $i++;
                    $level=$this->model_xls_pricelist_helper_models->getCategoryLevel($filtred_category['category_id'], 0, $this->xls_pricelist_language['language_id']);
                    $path=$this->model_xls_pricelist_helper_models->getCategoryPath($filtred_category['category_id'], array(), $this->xls_pricelist_language['language_id']);
                    //$level  =2;
                    //$path[]=$CCategory['category_id'];
                    //print $path."<br>";
                    $level1=$level;
                    if($level>2)$level1=2;

                    $this->load->model('catalog/product'); // подключаем модель работы с товаром


                    $parts = $this->model_xls_pricelist_helper_models->getCategoryName($filtred_category['category_id']); // используя нашу функцию пишем древо категорий в такой же

                    $this->sheet->setCellValueByColumnAndRow(0+$this->delim, $i, html_entity_decode(/*$filtred_category['name']*/$parts, ENT_QUOTES, 'UTF-8'));

                    for($j=0; $j<$this->poles_count +$skolko_razmerov; $j++){
                    ...
                    $this->sheet->mergeCells($this->sheet->getCellByColumnAndRow(0+$this->delim, $i)->getCoordinate().':'.$this->sheet->getCellByColumnAndRow($this->poles_count - 1+$this->delim +$skolko_razmerov, $i)->getCoordinate());
                    ...
                    $this->getProducts($i, $filtred_category['category_id'], $path, $cust_group, $level+1);
            ...
            if($method=='view'){
				...
				$objWriter->save(str_replace('/catalog/', '', DIR_APPLICATION) . "/price/Price_".$customer_group['name'].".xlsx");
                break;
            }else{
                include_once("PHPExcel/Writer/Excel2007.php");
                $objWriter = new PHPExcel_Writer_Excel2007($workbook);
    ...
    private function getProducts(&$i, $category_id, $path, $customer_group_id='', $level){
		$skolko_razmerov = 32; // размерный ряд
        list($sort, $order)=explode('-',$this->xls_pricelist_sort_order);
        ...
        foreach ($results as $result) { 
            ...
            for($j=0; $j<$this->poles_count; $j++){
                ...
                if($this->xls_pricelist_poles[$j]['bgcolor']){
                    $this->f_name = array(
                        ...
                        'fill' => array(
                            'type' => PHPExcel_Style_Fill::FILL_SOLID,
                            'startcolor' => array(
                                'argb' => '00'.($this->xls_pricelist_poles[$j]['bgcolor']?$this->xls_pricelist_poles[$j]['bgcolor']:'faf7dc'),
                            ),
                ...
                switch($this->xls_pricelist_poles[$j]['type']){
                    ...
                    case 'name':
                        ...
                        break;
                    case 'description':
                        $this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, $i, strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')));
                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, $i)->getCoordinate())->applyFromArray($this->f_name);
                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, $i)->getCoordinate())->getAlignment()->setWrapText(true);
                        break;
                    case 'quantity':
                        $this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, $i, $quantity);
                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, $i)->getCoordinate())->applyFromArray($this->f_name);
                        break;
                    case 'image':
                    ...
                    case 'price':
                        ...
                        $this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, $i, $price?$price:'');
                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, $i)->getCoordinate())->applyFromArray($this->f_name);
                        break;
                    case 'special':
                        ...
                        $this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, $i, $special?$special:'');
                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, $i)->getCoordinate())->applyFromArray($this->f_name);
                        break;
                    default:
                        $this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, $i, ($result[$this->xls_pricelist_poles[$j]['type']])?$result[$this->xls_pricelist_poles[$j]['type']]:'');
                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, $i)->getCoordinate())->applyFromArray($this->f_name);
            ...
            if($this->xls_pricelist_use_options){
                foreach ($this->model_xls_pricelist_helper_models->getProductOptions($result['product_id'], $this->xls_pricelist_language['language_id']) as $option) {
                            ...
                            for($j=0; $j<$this->poles_count; $j++){
                                ...
                                switch($this->xls_pricelist_poles[$j]['type']){
                                    case 'name':
										...
										/*
                                        $this->sheet->setCellValueByColumnAndRow($j+$this->delim, $i, html_entity_decode($option['name'].': '.$option_value['name'], ENT_QUOTES, 'UTF-8'));
                                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim, $i)->getCoordinate())->applyFromArray($this->f_name);
                                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim, $i)->getCoordinate())->getAlignment()->setWrapText(true);
                                        if($this->xls_pricelist_product_link){
                                            $this->sheet->getCellByColumnAndRow($j+$this->delim, $i)->getHyperlink()->setUrl(str_replace('&amp;', '&', $this->url->link('product/product', 'path=' . $path . '&product_id=' . $result['product_id'])));
					...
					// тут жесткий косяк с детской одеждой, т.к. она заведена по годам 6-12
					// в итоге получаем 6-26=-22 и и координата ячейки улетает за пределы листа, что вызывает fatal error при исполнении
					// российская таблица детских размеров с 6 лет: 5-6 лет 32, 6-8 лет 34, 9-10 лет 36, 11-2 лет 38
					// примерно тоже самое нужно внести в 1С для option_id=5 в таблице oc_option_value

					if ($option_value['name']<28) $option_value['name'] += 22;

					$option_col = $option_value['name'] - 26;


                                        $this->sheet->setCellValueByColumnAndRow($option_col, $i, $quantity);
                                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($option_col, $i)->getCoordinate())->applyFromArray($this->f_quantity);
                                        break;
                                    case 'image':
                                    ...
                                    case 'special':
                                       /* if ((float)$result['special']) {
                                        ...
                                        break;
                                    case 'weight':
                                       /* $this->sheet->setCellValueByColumnAndRow($j+$this->delim+$skolko_razmerov, $i, ($result[$this->xls_pricelist_poles[$j]['type']]+$option_weight)!=0?$result[$this->xls_pricelist_poles[$j]['type']]+$option_weight:'');
                                        $this->sheet->getStyle($this->sheet->getCellByColumnAndRow($j+$this->delim+$skolko_razmerov, $i)->getCoordinate())->applyFromArray($this->f_name);*/
                                        break;
                                    default:
