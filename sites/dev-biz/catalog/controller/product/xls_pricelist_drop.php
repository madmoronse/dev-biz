<?php
class ControllerProductXlsPricelistDrop extends Controller {
	...
	$filename = str_replace('/catalog/', '', DIR_APPLICATION) . "/price/Price_Drop.xlsx";
	if (time() - @filemtime($filename) < 60*2) exit();
        ...
        if (!extension_loaded('zip')) {
            error_log('Enable zip module in PHP configurations (php.ini or so)');
            error_log('Note: dl(...) is deprecated (and actually removed from PHP)');
            if (!dl('zip.so')) {
