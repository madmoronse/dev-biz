...
class ControllerModuletestimonial extends Controller {
	...
	protected function index($setting) {
		...
		foreach ($results as $result) {
			...
			if (mb_strlen($result['title'],'UTF-8')>40)
					$result['title'] = mb_substr($result['title'], 0, 37, 'UTF-8').'...';

			if (mb_strlen($result['name'],'UTF-8')>40)
					$result['name'] = mb_substr($result['name'], 0, 37, 'UTF-8').'...';

			if (mb_strlen($result['city'],'UTF-8')>40)
					$result['city'] = mb_substr($result['city'], 0, 37, 'UTF-8').'...';
					
					if (mb_strlen($result['description'],'UTF-8')>120)
					$result['description'] = mb_substr($result['description'], 0, 117, 'UTF-8').' ...';
