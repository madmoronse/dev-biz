<?php
class ControllerCheckoutfCart extends Controller {
	private $error = array();

	public function index() {
		$this->language->load('checkoutf/cart');
    ...
    if ((isset($this->request->post['incdec']))&&(!empty($this->request->post['pkey']))) {
      ...
      return;
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$this->data['action'] = $this->url->link('checkoutf/cart');
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
        		...
        		$this->data['products'][] = array(
					...
					'remove'   => $this->url->link('checkoutf/cart', 'remove=' . $product['key'])
				);
      		}
			...
			if (!empty($this->session->data['vouchers'])) {
				foreach ($this->session->data['vouchers'] as $key => $voucher) {
					$this->data['vouchers'][] = array(
						...
						'remove'      => $this->url->link('checkoutf/cart', 'remove=' . $key)
					);
				}
			}
			...
			$this->data['checkout'] = $this->url->link('checkoutf/checkout', '', 'SSL');

			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkoutf/cart.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/checkoutf/cart.tpl';
			} else {
				$this->template = 'default/template/checkoutf/cart.tpl';
			}
	...
	protected function validateCoupon() {
		$this->load->model('checkoutf/coupon');
	...
	protected function validateVoucher() {
		$this->load->model('checkoutf/voucher');
	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['manufacturer'] . ' ' . $product_info['name'], $this->url->link('checkoutf/cart'));
