	...
	class ControllerpavblogBlog extends Controller {
		...
		public function index() {  
			...
			if ($blog) {
				...
				if($blog['blog_products']){
                    $blog['blog_products'] = str_replace(" ", "", $blog['blog_products']);
                    $blog_products = array_diff(explode("\r\n", $blog['blog_products']), array(''));
                    ...
                    foreach ($blog_products as $product_id) {
                        ...
                        if ($product_info) {
							...
							if (($promo_settings['total_discount_status'] && $this->customer->getCustomerGroupId() < 2) || ($this->customer->getId() == $promo_settings['total_discount_test_user'] && $this->customer->getId() != "")) {
                                if (!empty($total_discount_products)) {
                                    if (in_array($product_info['product_id'], $total_discount_products)) {
                                        $special = $this->currency->format(round(($product_info['price'] - $product_info['price']/100*$promo_settings['total_discount_conditions'])/10)*10);
                                    }
                                } else{
                                    $special = $this->currency->format(round(($product_info['price'] - $product_info['price']/100*$promo_settings['total_discount_conditions'])/10)*10);
                                }
                            ...
                            $this->data['blog_products'][] = array(
                                'product_id' => $product_info['product_id'],
                                'thumb'   	 => $image,
                                'name'    	 => $product_info['fullname'],
                                'sku'        => $product_info['sku'],
                                'price'   	 => str_replace('ք','руб.',$price),
                                'special' 	 => $special,
                                'rating'     => $rating,
                                'manufacturer'     => $product_info['manufacturer'],
                                'reviews'    => sprintf($this->language->get('text_reviews'), (int)$product_info['reviews']),
                                'href'    	 => $this->url->link('product/product', 'product_id=' . $product_info['product_id']),
                                'href_manufacturer'    	 => $this->url->link('product/product', 'product_id=' . $product_info['product_id']),
                            );
                        }
                    }
				...
				$width = $this->mparams->get('general_lwidth');				
				list($width_orig, $height_orig) = getimagesize(DIR_IMAGE . $blog['image']);
				if ($width_orig>$width){
					...
					$blog['thumb_large'] = $this->model_tool_image->resize($blog['image'], $width, $height,'w');
				...
				if( count($blog['images']) > 0 ) {
					foreach ($blog['images'] as $key => $image) {
						$blog['images'][$key]['image_small'] = $this->model_tool_image->resize("data/Blog/pavimages/".$image['image'], 72, 55, 'w');
						$blog['images'][$key]['image_org'] = $this->model_tool_image->resize("data/Blog/pavimages/".$image['image'], 530, 386, 'w');
					}
				}
				...
				$this->data['breadcrumbs'][] = array(
					...
					'text'      => $this->language->get('Страница не найдена'),
					'href'      => $this->url->link('information/information', 'category_id=' . $category_id),
					'separator' => $this->language->get('text_separator')
				);
					
				//$this->document->setTitle($this->language->get('text_error'));
				$this->document->setTitle($this->language->get('Страница не найдена'));
				
				//$this->data['heading_title'] = $this->language->get('text_error');
				$this->data['heading_title'] = $this->language->get('Страница не найдена');

				//$this->data['text_error'] = $this->language->get('text_error');
				$this->data['text_error'] = $this->language->get('Страница не найдена');
