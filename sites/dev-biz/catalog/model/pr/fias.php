<?php
    class ModelPrFias extends Model {
        ...
        public function findFiasByName($term) {
            ...
            return $this->db->query("SELECT CONCAT_WS(', ',
                                        ...
                                        FROM fias f1
                                          ...
                                          ORDER BY f1.level, f2.level, f3.level")->rows;
        }

        public function getFiasById($fias_id) {

            return $this->db->query("SELECT f1.postalcode f1_postalcode, CONCAT(f1.shortname, ' ', f1.offname) f1_name,
                                            f1.level f1_level, f1.fias_id f1_fias_id,
                                      ...
                                      FROM fias f1
                                        LEFT JOIN fias f2 ON f2.aoguid = f1.parentguid
                                        LEFT JOIN fias f3 ON f3.aoguid = f2.parentguid
                                        LEFT JOIN fias f4 ON f4.aoguid = f3.parentguid
                                      WHERE f1.fias_id = " . (int)$fias_id . "
                                       AND (f1.level = 6 OR f1.level = 4 OR (f1.level = 1 AND f1.shortname = 'г.'))")->row;
        }
        ...
        public function getCountryByIsoCode($iso_code) {
            ...
            $country = $this->db->query('SELECT c.country_id
                                        FROM ' . DB_PREFIX . 'country c
                                        WHERE c.iso_code_3 = "' . $this->db->escape($iso_code) . '"');

            return $country->row ? $country->row['country_id'] : null;
        }
            ...
            if (!empty($filter['city_name'])) {
                return $this->db->query("SELECT f1.postalcode f1_postalcode, CONCAT(f1.shortname, ' ', f1.offname) f1_name,
                                      ...
                                      FROM fias f1
                                        ...
                                        LEFT JOIN fias f3 ON f3.aoguid = f2.parentguid
                                        LEFT JOIN fias f4 ON f4.aoguid = f3.parentguid
                                      WHERE f1.offname = '" . $filter['city_name'] . "'
                                        AND (f3.offname = '" . $filter['country_name'] . "' OR f2.offname = '" . $filter['country_name'] . "' OR f4.offname = '" . $filter['country_name'] . "')
                ...
                return $this->db->query("SELECT f1.offname f1_name, f1.level f1_level, f1.fias_id f1_fias_id
                                      FROM fias f1
                                      WHERE f1.offname = '" . $filter['country_name'] . "' AND f1.level = 0")->row;
            }
        }
    }