...
$FULL_CATALOG=$DB->select('SELECT * FROM `om_sale_towall` ORDER BY `id` ASC LIMIT 40,5');
...
foreach ($FULL_CATALOG as $rowSKU=>$SKU) {
	...
	$IMAGES=$DB->select('SELECT * FROM `oc_product_image` WHERE `product_id`=?',$SKU[om_sku]);//выбрали фотки к нему
		...
		//грузим фотки для стены для одного арта объединяем их в пост

			$WALL_IMAGES = array();
		
			$WALL_IMAGES[] = $vk->uploadImageToWall($main_image_path,$group_id, $descr);//грузим главное фото для стены


			foreach($IMAGES as $rowIMG=>$IMG) {

				$image_path=$_SERVER['DOCUMENT_ROOT'].'/image/'.$IMG[image];//доп фотки если есть
				$WALL_IMAGES[] = $vk->uploadImageToWall($image_path,$group_id, $descr);//доп фотки если есть
			}
			

			$WALLIMGS=implode(',',$WALL_IMAGES);
			//$WALLIMGS=$WALL_IMAGES[0];


			print $WALLIMGS . '<br>';

	
			$post_to_wall = $vk->postImgToWall($WALLIMGS, $group_id, $descr);
